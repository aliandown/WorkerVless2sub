...
let subconverter = "psub.alianmsnqq.workers.dev"; //在线订阅转换后端，目前使用CM的订阅转换功能。支持自建psub 可自行搭建https://github.com/bulianglin/psub
	...
	async fetch (request, env) {
		...
		*/
		...
		if (!userAgent.includes('subconverter') && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
			...
			subconverterUrl = `https://${subconverter}/sub?target=clash&url=${encodeURIComponent(subconverterUrl)}&insert=false&config=${encodeURIComponent(subconfig)}&emoji=true&list=true&tfo=false&scv=true&fdn=false&sort=false&new_name=true&udp=true`;
			...
			subconverterUrl = `https://${subconverter}/sub?target=singbox&url=${encodeURIComponent(subconverterUrl)}&insert=false&config=${encodeURIComponent(subconfig)}&emoji=true&list=true&tfo=false&scv=true&fdn=false&sort=false&new_name=true&udp=true`;
		} else {
			...
			if(noTLS == 'true' && 协议类型 == 'VLESS'){
				...
				notlsresponseBody = uniqueAddressesnotls.map(address => {
					...
					const vlessLink = `vless://${uuid}@${address}:${port}?encryption=none&allowlnsecure=false&security=&type=${type}&host=${host}&path=${encodeURIComponent(path)}#${encodeURIComponent(addressid + EndPS)}`;
			...
			if (协议类型 == 'Trojan' && (userAgent.includes('surge') || (format === 'surge' && !userAgent.includes('subconverter')) ) && !userAgent.includes('cf-workers-sub')) {
				...
				subconverterUrl =  `https://${subconverter}/sub?target=surge&ver=4&url=${encodeURIComponent(TrojanLinksJ8)}&insert=false&config=${encodeURIComponent(subconfig)}&emoji=true&list=true&xudp=false&udp=true&tfo=false&expand=true&scv=true&fdn=false`;
			} else {
...
function surge(content, url) {
	...
	for (let x of 每行内容) {
		if (x.includes('= trojan,')) {
			...
			const 备改内容 = `skip-cert-verify=false, tfo=false, udp-relay=true`;
			const 正确内容 = `skip-cert-verify=false, ws=true, ws-path=/?ed=2560, ws-headers=Host:"${host}", tfo=false, udp-relay=true`;
