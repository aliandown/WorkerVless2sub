...
function surge(content, url) {
	...
	for (let x of 每行内容) {
		if (x.includes('= trojan,')) {
			...
			const 备改内容 = `skip-cert-verify=false, tfo=false, udp-relay=false, udp=true`;
			const 正确内容 = `skip-cert-verify=false, ws=true, ws-path=/?ed=2560, ws-headers=Host:"${host}", tfo=false, udp-relay=false, udp=true`;
